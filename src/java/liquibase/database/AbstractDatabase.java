...
/**
 * AbstractDatabase is extended by all supported databases as a facade to the underlying database. The
 * physical connection can be retrieved from the AbstractDatabase implementation, as well as any
...
public abstract class AbstractDatabase implements Database
{
    ...
    /**
     * Returns the database-specific datatype for the given column configuration. This method will convert
     * some generic column types (e.g. boolean, currency) to the correct type for the current database.
     */
    @Override
    ...
    // Get the type from the Connection MetaData (use the MetaData to translate from java.sql.Types to
    // DB-specific type)
     ...
     * <p/>
     * Note: many databases accept date literals in ISO8601 format with the 'T' replaced with a space. Only
     * databases which do not accept these strings should need to override this method.
     * <p/>
    ...
    {
        return new InsertStatement(getDefaultSchemaName(), getDatabaseChangeLogLockTableName()).addColumnValue("ID", 1).addColumnValue("LOCKED",
                Boolean.FALSE);
    }
    ...
    protected SqlStatement getCreateChangeLogLockSQL()
    {
        return new CreateTableStatement(getDefaultSchemaName(), getDatabaseChangeLogLockTableName())
                ...
                .addColumn("LOCKGRANTED", getDateTimeType().getDataTypeName()).addColumn("LOCKEDBY", "VARCHAR(255)");
    }
    ...
    protected SqlStatement getCreateChangeLogSQL()
    {
        return new CreateTableStatement(getDefaultSchemaName(), getDatabaseChangeLogTableName())
                ...
                .addColumn("DATEEXECUTED", getDateTimeType().getDataTypeName(), new NotNullConstraint()).addColumn("MD5SUM", "VARCHAR(32)")
                .addColumn("DESCRIPTION", "VARCHAR(255)").addColumn("COMMENTS", "VARCHAR(255)").addColumn("TAG", "VARCHAR(255)")
    ...
    {
        return new RawSqlStatement(("SELECT LOCKED FROM " + escapeTableName(getDefaultSchemaName(), getDatabaseChangeLogLockTableName()) + " WHERE "
                + escapeColumnName(getDefaultSchemaName(), getDatabaseChangeLogLockTableName(), "ID") + "=1"));
    }
    ...
    /**
     * This method will check the database ChangeLog table used to keep track of the changes in the file. If
     * the table does not exist it will create one otherwise it will not do anything besides outputting a log
     * message.
     */
    @Override
    public void checkDatabaseChangeLogTable() throws JDBCException
    {
        ...
        try
        {
            ...
            {
                checkColumnsRS = connection.getMetaData().getColumns(convertRequestedSchemaToCatalog(getDefaultSchemaName()),
                        convertRequestedSchemaToSchema(getDefaultSchemaName()), getDatabaseChangeLogTableName(), null);
                ...
                if (!hasDescription)
                {
                    statementsToExecute.add(new AddColumnStatement(getDefaultSchemaName(), getDatabaseChangeLogTableName(), "DESCRIPTION",
                            "VARCHAR(255)", null));
                }
                if (!hasTag)
                {
                    statementsToExecute.add(new AddColumnStatement(getDefaultSchemaName(), getDatabaseChangeLogTableName(), "TAG", "VARCHAR(255)",
                            null));
                }
                if (!hasComments)
                {
                    statementsToExecute.add(new AddColumnStatement(getDefaultSchemaName(), getDatabaseChangeLogTableName(), "COMMENTS",
                            "VARCHAR(255)", null));
                }
                if (!hasLiquibase)
                {
                    statementsToExecute.add(new AddColumnStatement(getDefaultSchemaName(), getDatabaseChangeLogTableName(), "LIQUIBASE",
                            "VARCHAR(255)", null));
                }
            ...
            else if (!changeLogCreateAttempted)
            {
                ...
                {
                    throw new JDBCException("Cannot create " + escapeTableName(getDefaultSchemaName(), getDatabaseChangeLogTableName())
    ...
    public boolean doesChangeLogLockTableExist() throws JDBCException
    {
        ...
        try
        {
            rs = connection.getMetaData().getTables(convertRequestedSchemaToCatalog(getDefaultSchemaName()),
                    convertRequestedSchemaToSchema(getDefaultSchemaName()), getDatabaseChangeLogLockTableName(), new String[]
                    {
                        "TABLE"
                    });
            return rs.next();
    ...
    /**
     * This method will check the database ChangeLogLock table used to keep track of if a machine is updating
     * the database. If the table does not exist it will create one otherwise it will not do anything besides
    ...
    public void checkDatabaseChangeLogLockTable() throws JDBCException
    {
        ...
        {
            RawSqlStatement selectStatement = new RawSqlStatement("SELECT COUNT(*) FROM "
                    + escapeTableName(getDefaultSchemaName(), getDatabaseChangeLogLockTableName()) + " WHERE ID=1");
            try
            {
    ...
    public void dropDatabaseObjects(String schema) throws JDBCException
    {
        try
        {
            ...
            if (snapshot.hasDatabaseChangeLogTable())
            {
                ...
                clearChangeLogChange
                        .setSql("DELETE FROM " + escapeTableName(convertRequestedSchemaToSchema(schema), getDatabaseChangeLogTableName()));
    ...
    public void tag(String tagString) throws JDBCException
    {
        try
        {
            int totalRows = this.getJdbcTemplate().queryForInt(
                    new RawSqlStatement("SELECT COUNT(*) FROM " + escapeTableName(getDefaultSchemaName(), getDatabaseChangeLogTableName())),
                    new ArrayList<SqlVisitor>());
            if (totalRows == 0)
            {
            ...
            // this.getJdbcTemplate().queryForObject(createChangeToTagSQL(), Timestamp.class);
    ...
    public boolean doesTagExist(String tag) throws JDBCException
    {
        int count = this.getJdbcTemplate().queryForInt(
                new RawSqlStatement("SELECT COUNT(*) FROM " + escapeTableName(getDefaultSchemaName(), getDatabaseChangeLogTableName())
                        + " WHERE TAG='" + tag + "'"), new ArrayList<SqlVisitor>());
        return count > 0;
    }
    ...
    public Object convertDatabaseValueToJavaObject(Object defaultValue, int dataType, int columnSize, int decimalDigits) throws ParseException
    {
        ...
        {
            return convertToCorrectJavaType(((String) defaultValue).replaceFirst("^'", "").replaceFirst("'$", ""), dataType, columnSize,
                    decimalDigits);
        }
        else
        {
    ...
    public String escapeTableName(String schemaName, String tableName)
    {
        ...
        {
            return getObjectEscapeCharacter() + schemaName + getObjectEscapeCharacter() + "." + getObjectEscapeCharacter() + tableName
                    + getObjectEscapeCharacter();
        }
    }
    ...
    public String escapeSequenceName(String schemaName, String sequenceName)
    {
        ...
        {
            return getObjectEscapeCharacter() + schemaName + getObjectEscapeCharacter() + "." + getObjectEscapeCharacter() + sequenceName
                    + getObjectEscapeCharacter();
        }
    }
    ...
    public boolean isColumnAutoIncrement(String schemaName, String tableName, String columnName) throws SQLException, JDBCException
    {
        ...
        try
        {
            selectRS = getConnection().createStatement().executeQuery(
                    "SELECT " + escapeColumnName(schemaName, tableName, columnName) + " FROM " + escapeTableName(schemaName, tableName)
                            + " WHERE 1 = 0");
        ...
        else
        {
            if (foundRan.getMd5sum() == null)
            {
                try
                {
                    ...
                    DatabaseConnection connection = getConnection();
                    PreparedStatement updatePstmt = connection.prepareStatement("UPDATE "
                            + escapeTableName(getDefaultSchemaName(), getDatabaseChangeLogTableName())
                            + " SET MD5SUM=? WHERE ID=? AND AUTHOR=? AND FILENAME=?");
    ...
    {
        String dateValue = getCurrentDateTimeFunction();
        String sql = "UPDATE " + escapeTableName(getDefaultSchemaName(), getDatabaseChangeLogTableName()) + " SET DATEEXECUTED=" + dateValue
                + ", MD5SUM='?' WHERE ID='?' AND AUTHOR='?' AND FILENAME='?'";
    ...
    {
        String sql = "DELETE FROM " + escapeTableName(getDefaultSchemaName(), getDatabaseChangeLogTableName())
